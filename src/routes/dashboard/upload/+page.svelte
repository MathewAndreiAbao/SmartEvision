    ...
    async function fetchInitialData(userProfile: Profile) {
        ...
        if (loads.length > 0) {
            ...
            if (!teachingLoadId) {
              ...
              if (navigator.onLine && !$profile?.role?.includes('School Head') && !$profile?.role?.includes('District Supervisor')) {
...
{#if requiresTeachingLoadSelection($profile?.role || '', docType)}
                                <!-- Week -->
                                <div class="space-y-2">
                                    <span
                                        class="text-[10px] font-bold text-text-muted uppercase tracking-wide ml-1"
                                    ...
                                    >
                                    <button
                                        onclick={() => (showWeekPicker = teachingLoads.length > 0)}
                                        disabled={teachingLoads.length === 0}
                                        class="w-full p-4 rounded-md bg-surface-muted border border-border-subtle transition-all text-left flex items-center justify-between group {teachingLoads.length === 0 ? 'opacity-60 cursor-not-allowed' : 'hover:border-gov-blue/30'}"
                                    >
<div>
                            ...
                            {:else}
                            <!-- Week not applicable for ISP/ISR uploads -->
                            <div
                                class="p-4 text-center text-text-muted italic text-xs"
                                role="alert"
                            >
                                <p class="font-medium">Week selection not applicable for {docType === 'ISP' ? 'Instructional Supervisory Plan' : 'Instructional Supervisory Report'}</p>
                            </div>
                        {/if}
                                        </div>
                                        <div
