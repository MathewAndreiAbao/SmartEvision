        ...
        >
            ...
            >
                <div class="flex items-start gap-3 sm:gap-4 flex-1 min-w-0">
                    <!-- Icon -->
                    <div
                        class="flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center {isHighSeverity
                            ? ‘bg-gov-red/20 text-gov-red’
                    ...
                    <div class="min-w-0 flex-1">
                        <h3 class="font-bold text-text-primary text-sm sm:text-base flex items-center gap-2 flex-wrap">
                            Compliance Alerts
                            <span
                                class="inline-flex items-center justify-center px-2 py-1 rounded-lg text-xs font-bold {isHighSeverity
                ...
                <div class="flex-shrink-0">
                    <div
                        class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-black/5 transition-colors duration-200"
                    >
                ...
                <div class="border-t border-border-subtle bg-surface-muted/30" transition:slide={{ duration: 250 }}>
                    <div class="max-h-[400px] overflow-y-auto cedims-scroll p-3 sm:p-4 space-y-2.5">
                        {#each alerts as alert}
                            <div
                                class="p-3 sm:p-4 rounded-lg bg-surface-white border border-border-subtle hover:border-gov-blue/40 transition-all duration-200"
                            >
                                <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                                    ...
                                    <div class="flex items-center justify-between sm:flex-col sm:items-end gap-3 flex-shrink-0">
                                        <span
                                            class="text-xs font-bold uppercase px-3 py-1.5 rounded-lg {alert.severity === ‘high’
                                                ? ‘bg-gov-red text-white’
                                                : ‘bg-gov-gold text-white’}"
                                        >
                                            {alert.severity}
