...
<div class="w-full">
    ...
    <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 mb-6 pb-4 border-b border-border-subtle">
        ...
        {#if isPdf && pdfDoc && isReviewer}
            <div class="flex flex-wrap items-center gap-2 bg-surface-muted/50 p-1.5 rounded-xl border border-border-subtle">
                <button
                    onclick={() => {
                        selectedTool = selectedTool === 'highlight' ? null : 'highlight';
                        highlightColor = '#EAB308';
                    }}
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all {selectedTool === 'highlight' ? 'bg-yellow-500 text-white shadow-md' : 'text-text-muted hover:bg-white'}"
                >
                    <Highlighter size={14} />
                    Highlight
                </button>
                ...
                <button
                    ...
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all {selectedTool === 'comment' ? 'bg-blue-500 text-white shadow-md' : 'text-text-muted hover:bg-white'}"
                >
                ...
                <button
                    ...
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all {selectedTool === 'flag' ? 'bg-red-500 text-white shadow-md' : 'text-text-muted hover:bg-white'}"
                >
                    <AlertCircle size={14} />
                    Flag Issue
                </button>
    ...
    {:else if loadError || !isPdf}
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="md:col-span-2 space-y-4">
                <div class="bg-surface-muted/30 border border-border-subtle rounded-xl p-8 text-center">
                    ...
                    {#if isReviewer}
                        <div class="mt-6 max-w-md mx-auto text-left bg-white border border-border-subtle rounded-xl p-4">
                            <label class="block text-xs font-bold text-text-muted uppercase mb-2">Add Review Note</label>
                            <textarea
                            ...
                            <div class="flex justify-between items-center mt-3">
                                <select 
                                    ...
                                    class="text-xs font-semibold bg-surface-muted/50 border border-border-subtle p-1.5 rounded-lg text-text-primary"
                                >
            ...
            <div class="space-y-3">
                ...
                {:else}
                    {#each annotations as annotation}
                        <div class="p-4 border rounded-xl bg-white border-border-subtle flex flex-col gap-2">
    ...
    {:else}
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="lg:col-span-3 flex flex-col">
                <!-- PDF Controls Bar -->
                <div class="flex items-center justify-between p-3 bg-slate-800 text-white rounded-t-xl">
                    <div class="flex items-center gap-2">
                        <button
                            onclick={handlePrevPage}
                            disabled={currentPage === 1}
                            class="p-1.5 rounded-lg hover:bg-slate-700 disabled:opacity-30 transition-colors"
                        >
                            <ChevronLeft size={16} />
                        </button>
                    ...
                    <!-- Zoom Controls -->
                    <div class="flex items-center gap-1.5 bg-slate-700/50 px-2 py-1 rounded-lg">
                        <button 
                            onclick={handleZoomOut} 
                            disabled={zoom <= 0.5}
                            class="p-1 hover:bg-slate-600 rounded disabled:opacity-30 transition-colors"
                        >
                            <ZoomOut size={14} />
                        </button>
                ...
                <div 
                    ...
                    class="relative flex justify-center bg-slate-100 border-x border-b border-border-subtle overflow-auto p-4 select-none min-h-[500px] max-h-[800px] rounded-b-xl"
                >
                    <div 
                        class="relative shadow-xl border border-gray-300/60 bg-white"
                    ...
                    >
                        ...
                        {#if hoveredAnnotation}
                            <div 
                                class="absolute z-30 pointer-events-none bg-slate-900 text-white text-xs rounded-lg p-3 shadow-xl max-w-xs flex flex-col gap-1 -translate-x-1/2"
                                style="left: {tooltipX}px; top: {tooltipY - 60}px;"
                                transition:fade={{ duration: 100 }}
                            >
                                <div class="flex items-center gap-1.5">
                                    <span class="font-bold uppercase text-[9px] px-1 bg-white/20 rounded">
                        ...
                        {#if activeClick && activeClick.page === currentPage}
                            <div 
                                class="absolute z-20 bg-white border border-border-subtle shadow-2xl rounded-xl p-4 w-72 flex flex-col gap-3 -translate-x-1/2"
            ...
            <div class="lg:col-span-1 flex flex-col bg-white border border-border-subtle rounded-xl p-4">
                    ...
                    {:else}
                        {#each annotations as annotation}
                            <button
                                ...
                                class="w-full text-left p-3.5 border rounded-xl bg-white hover:border-gov-blue/50 hover:shadow-sm transition-all duration-200 flex flex-col gap-2 relative group"
                            >
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-1.5">
